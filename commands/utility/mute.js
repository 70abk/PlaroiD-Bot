...
const SCHEDULE_FILE = join(__dirname, config.scheduleFilePath);
const TIMEOUT_FILE = join(__dirname, config.timeoutFilePath);
import { v4 as uuidv4 } from 'uuid';

export const data = new SlashCommandBuilder()
    .setName('뒤주')
    .setDescription('서버 멤버에게 활동 정지를 지급합니다.')
    .addUserOption(option => option.setName('이름')
        .setDescription('활동 정지를 지급할 멤버')
        .setRequired(true)
    )
    .addStringOption(option => option.setName('기한')
...
export async function execute(interaction) {
    ...
    promises.writeFile(SCHEDULE_FILE, JSON.stringify(schedules, null, 2), 'utf8');
...
function loadSchedules() {
    try {
        if (existsSync(SCHEDULE_FILE)) {
            const data = promises.readFile(SCHEDULE_FILE, 'utf8');
...
function scheduleTask(task) {
    ...
    if (delay > 0) {
        ...
        const tID = setTimeout(async () => {
            ...
            promises.writeFile(SCHEDULE_FILE, JSON.stringify(schedules, null, 2));
        }, delay);
        timeoutMap.set(task.userId, tID[Symbol.toPrimitive]('number'));
        promises.writeFile(TIMEOUT_FILE, JSON.stringify(Object.fromEntries(timeoutMap)));
    } else {
        const schedules = loadSchedules().filter(s => s.id !== task.id);
        promises.writeFile(SCHEDULE_FILE, JSON.stringify(schedules, null, 2))
        unmute(task);
...
function loadTimeout() {
    try {
        const data = JSON.parse(promises.readFile(TIMEOUT_FILE, 'utf8'));
